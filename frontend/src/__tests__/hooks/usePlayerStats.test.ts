...
describe("usePlayerStats", () => {
  ...
  it("appends from and to query params when provided", async () => {
    ...
    const { result } = renderHook(
      () => usePlayerStats(1, null, "2026-01-01", "2026-02-21"),
      { wrapper },
    );
    ...
    expect(api.apiFetch).toHaveBeenCalledWith(
      "/statistics/players/1?from=2026-01-01&to=2026-02-21",
    );
  });
  ...
  it("appends from/to together with playerGroup", async () => {
    ...
    const { result } = renderHook(
      () => usePlayerStats(1, 5, "2026-01-01", "2026-02-21"),
      { wrapper },
    );
    ...
    expect(api.apiFetch).toHaveBeenCalledWith(
      "/statistics/players/1?from=2026-01-01&playerGroup=5&to=2026-02-21",
    );
  });
  ...
  it("appends only from when to is null", async () => {
    ...
    const { result } = renderHook(
      () => usePlayerStats(1, null, "2026-01-01", null),
      { wrapper },
    );
    ...
    expect(api.apiFetch).toHaveBeenCalledWith(
      "/statistics/players/1?from=2026-01-01",
    );
  });
});
